...
public final class ScheduledFutureTask<V> extends PromiseTask<V> implements ScheduledFuture<V>, PriorityQueueNode {
    ...
    /**
     * 翻译过来是周期的意思，这个属性自然会和循环执行的定时任务有关
     * 当它是0的时候，该任务不会循环执行
    ...
    /**
     * 比较器的具体使用逻辑，就是比较两个定时任务执行事件的大小，谁的时间小，谁就排在任务队列的前面
     */
    @Override
                ...
                if (!isCancelled()) {
                    ...
                    if (!executor().isShutdown()) {
                        ...
                        if (p > 0) {
                            ...
                            // 因为大于0的时候对应scheduleAtFixedRate，该方法的下次执行时间就是这次的执行时间加上固定周期
                            // 如果延时了，那任务结束后并且超过下一次定时任务执行的时间了，那该定时任务就会立刻被执行
                            deadlineNanos += p;
                        } else {
                            // 小于0则意味着对应scheduleWithFixedDelay方法，上一次任务结束后，如果超时了
                            // 它会再次延时固定的周期，然后再执行下一个任务
                            // 这里p本身就是负数，相减为加当前时间减去开始时间，然后加上固定的延时，这就意味着不管任务是否超时，都要加上延时
                            deadlineNanos = nanoTime() - p;
                        }
                        // 如果该任务还没被取消
                        if (!isCancelled()) {
                            // 得到定时任务的队列，把定时任务放进队列中，因为该任务是循环执行的，计算出了下一次的执行时间了，所以要重新放回队列中
    ...
    /**
     * 下面这两个方法会在DefaultPriorityQueue类中被调用
     */
    @Override
