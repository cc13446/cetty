...
import static com.cc.cetty.priority.PriorityQueueNode.INDEX_NOT_IN_QUEUE;
...
public final class DefaultPriorityQueue<T extends PriorityQueueNode> extends AbstractQueue<T> implements PriorityQueue<T> {
    ...
    /**
     * 初始化一个PriorityQueueNode类型的数组，该PriorityQueueNode类型实际上就ScheduledFutureTask
     * 因为只有ScheduledFutureTask实现了PriorityQueueNode接口
     */
    ...
    public DefaultPriorityQueue(Comparator<T> comparator, int initialSize) {
        this.comparator = AssertUtils.checkNotNull(comparator);
    ...
    /**
     * 移动，也就是重新排列任务队列中的定时任务的优先级，这里面用到的就是二分法
     */
