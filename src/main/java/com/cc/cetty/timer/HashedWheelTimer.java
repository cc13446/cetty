...
/**
 * netty实现的时间轮是单线程的，这意味着一旦时间轮中的任务有阻塞的话，整个时间轮中任务的执行可能都会受到影响
 *
 * @author: cc
 * @date: 2023/11/06
 **/
...
public class HashedWheelTimer implements Timer {
    ...
    /**
     * 初始化该时间轮的runnable，时间轮是单线程的，单线程执行的是该runnable，其中的run方法就是时间轮的核心逻辑
     */
    ...
    /**
     * 本来在时间轮中用的是无锁队列，要引入新的jar包，其实性能也体现在这里
     * 但这节课我们只是讲解时间轮的调用逻辑，所以就把这两个高性能无锁队列换成下面Java的队列了
     */
     ...
     * @param tickDuration       时间间隔，也就是时间轮的刻度大小
     * @param unit               单位
     * @param ticksPerWheel      时间轮的容量 也就是时间轮数组槽位的个数
     * @param leakDetection      内存泄漏检测
        ...
        switch (WORKER_STATE_UPDATER.get(this)) {
            case WORKER_STATE_INIT:
                ...
                if (WORKER_STATE_UPDATER.compareAndSet(this, WORKER_STATE_INIT, WORKER_STATE_STARTED)) {
                    // 启动work线程，该线程一旦启动，就会执行任务，所以核心在work线程要执行的runnable的run方法内
                    workerThread.start();
                }
                break;
            // 如果启动了就什么也不做
    ...
    /**
     * 时间轮的工作线程，也就是执行定时任务的线程在创建第一个定时任务的时候被启动了，并且只启动一次
